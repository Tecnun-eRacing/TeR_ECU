 ...
 *      Author: pieroebs
 *
 *      Lógica de todo lo relacionado con el AS
 */
 ...
 *
 * -> gets the state
 * -> executes state changes transitions
 * -> execcutes permanents tasks associated with the current state
 *
 * */
...
void dv_statemachine() {
	...
	case AS_DRIVING:
		if (TeR.status.as_allowed) {
			...
			//requests freno
			TeR.asb_brake_req.brake =
					(TeR.dv_dynamic_req_2.asb_brake_req
							== TER_DV_DYNAMIC_REQ_2_ASB_BRAKE_REQ_ENABLED_CHOICE) ?
							TER_ASB_BRAKE_REQ_BRAKE_ENABLED_CHOICE :
							TER_ASB_BRAKE_REQ_BRAKE_DISABLED_CHOICE;
		} else {
			TeR.dv_system_status.steering_state =
			TER_DV_SYSTEM_STATUS_STEERING_STATE_UNAVAILABLE_CHOICE;
		}
