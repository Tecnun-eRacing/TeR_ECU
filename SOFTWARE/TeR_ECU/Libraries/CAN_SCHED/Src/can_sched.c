...
bool can_scheduler_insert_msg(uint8_t* msg, uint8_t len, uint32_t id, uint32_t period_ms, void (*callback)(CanMessage_t*)) {
    CanMessage_t can_msg = {.len = len, .id = id, .next_when=osKernelGetTickCount(), .period = period_ms, .callback = callback};
    memcpy(can_msg.content, msg, sizeof(can_msg.content));
    return can_scheduler_insert_built_msg(can_msg);
}

bool can_scheduler_insert_msg_with_timeout(uint8_t* msg, uint8_t len, uint32_t id, uint32_t period_ms, int32_t timeout, void (*callback)(CanMessage_t*)) {
    CanMessage_t can_msg = {.len = len, .id = id, .next_when=timeout, .period = period_ms, .callback = callback};
    memcpy(can_msg.content, msg, sizeof(can_msg.content));
    return can_scheduler_insert_built_msg(can_msg);
}

bool can_scheduler_insert_non_periodic_msg(uint8_t* msg, uint8_t len, uint32_t id, uint32_t timeout) {
    return can_scheduler_insert_msg_with_timeout(msg, len, id, -1, timeout + osKernelGetTickCount(), 0);
}
